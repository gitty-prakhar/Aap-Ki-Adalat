    ...
    <div className="max-w-4xl mx-auto flex flex-col md:flex-row gap-8">
      <div className="md:w-2/3">
        ...
        <div className="space-y-4 mb-8">
          {categories.map((cat) => (
            <motion.div
              ...
              whileTap={{ scale: 0.99 }}
              className={`p-4 border font-mono cursor-pointer transition-colors ${
        ...
        <button
          ...
          className="w-full py-4 bg-gold-500 text-dark-900 font-mono text-xs font-bold tracking-widest hover:bg-gold-400 transition-colors disabled:opacity-50 cursor-pointer"
        >
      ...
      <div className="md:w-1/3">
        <div className="bg-dark-800 border border-dark-700 p-6 sticky top-28">
          ...
          {jurorStatus ? (
            <div className="space-y-4 font-mono text-xs">
              <div className="flex justify-between border-b border-dark-700 pb-2">
                ...
                <span className={jurorStatus.isActive ? 'text-green-500' : 'text-red-500'}>
                  {jurorStatus.isActive ? 'ACTIVE' : 'INACTIVE'}
                </span>
              </div>
              <div className="flex justify-between border-b border-dark-700 pb-2">
                <span className="text-gray-500 uppercase">Staked Amount</span>
                <span className="text-white">{formatEther(jurorStatus.stake)} ETH</span>
              </div>
              <div className="flex justify-between border-b border-dark-700 pb-2">
                <span className="text-gray-500 uppercase">Category</span>
                <span className="text-white">
                  {jurorStatus.category.toString() === '0'
                    ? 'None'
                    : jurorStatus.category.toString()}
                </span>
              </div>
              <div className="flex justify-between border-b border-dark-700 pb-2">
                <span className="text-gray-500 uppercase">Locked?</span>
                <span className={jurorStatus.isLocked ? 'text-orange-500' : 'text-green-500'}>
                  {jurorStatus.isLocked
                    ? `YES (Case #${jurorStatus.lockedCase})`
                    : 'NO'}
                </span>
              </div>
              ...
              {jurorStatus.isActive && !jurorStatus.isLocked && (
                <button
                  ...
                  disabled={isPending || isConfirming}
                  className="w-full py-3 mt-4 border border-red-500/50 text-red-500 hover:bg-red-500/10 transition-colors block text-center uppercase tracking-widest cursor-pointer disabled:opacity-50"
                >
                  Unstake & Withdraw
                </button>
              )}
